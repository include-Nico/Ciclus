...
/* ==========================================================================
   CICLUS — Logica Applicazione (Stato, Configurazione, Dati)
   ========================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    try { 
        ...
        setGpsStatus('poor', 'Mappa offline'); 
    }
...
function applySettingsToUI() {
    ...
    el.mapThemeToggle.querySelectorAll('.segmented-option').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.theme === state.mapTheme);
    });
    
    el.mapRotateToggle.querySelectorAll('.segmented-option').forEach(btn => {
        btn.classList.toggle('active', btn.dataset.rotate === state.mapRotate);
    });
    
    if(state.mapRotate === 'off' && state.lastBearing !== null) {
...
function renderTappeMarkers() {
    ...
    if(tappeLayerGroup) {
        state.map.removeLayer(tappeLayerGroup);
    }
...
function deriveSpeedFromFixes(coords, timestamp) {
    ...
    const distance = haversineDistanceKm(state.lastPosition.latitude, state.lastPosition.longitude, coords.latitude, coords.longitude);
    return distance / dtHours;
}
...
function updateGpsQuality(accuracy) {
    if (accuracy <= 15) {
        setGpsStatus('live', 'Segnale ottimo'); 
    } else if (accuracy <= 35) {
        setGpsStatus('weak', 'Segnale discreto'); 
    } else {
        setGpsStatus('poor', 'Segnale debole');
    }
}
...
function setControlsMode(mode) {
    if (mode === 'idle') { 
        ...
        el.pauseBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none"><rect x="6" y="5" width="4" height="14" rx="1" fill="currentColor"/><rect x="14" y="5" width="4" height="14" rx="1" fill="currentColor"/></svg>`; 
    } else if (mode === 'paused') { 
        el.startBtn.classList.add('hidden'); 
        el.pauseBtn.classList.remove('hidden'); 
        el.stopBtn.classList.remove('hidden'); 
        el.pauseBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none"><path d="M7 5.5v13l11-6.5-11-6.5Z" fill="currentColor"/></svg>`; 
    }
}

/* ==========================================================================
   Cronologia Corse (Rides)
   ========================================================================== */
